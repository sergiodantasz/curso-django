...
class RecipeHomeViewTest(RecipeTestBase):
    ...
    def test_recipe_home_template_shows_no_recipe_was_found_if_no_recipe_was_found(
        self,
    ):
        response = self.client.get(reverse('recipes:home'))
        self.assertIn('No recipe was found. :(', response.content.decode('utf-8'))
    ...
    def test_recipe_home_template_loads_recipes(self):
        ...
        self.assertEqual(len(context_recipes), 1)  # type: ignore
    ...
    def test_recipe_home_is_paginated(self):
        self.create_recipe_in_batch(8)
        with patch('recipes.views.PER_PAGE', new=3):
            response = self.client.get(reverse('recipes:home'))
            recipes = response.context.get('recipes')
            paginator = recipes.paginator  # type: ignore
